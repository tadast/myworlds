...
(function () {
  ...
  // ---- roller (issue 28) ----
  // A roller lives on any land niche, so it needs no gate of its own. It joins the list here and
  // not in the line above, so the three impulse packages of issue 28 merge without a conflict.
  LOCO.land.push('roller');
  const PLAN = {
  ...
  // ---- roller (issue 28) ----
  // It stores what it has and spends the whole of it in one throw. The gate reads the steering
  // model and not the locomotion, so every impulse animal of issue 28 answers to it at once.
  ...
  const CLIMATE = pool([
    ...
    // ---- roller (issue 28) ----
    // The world sets the length of a throw, so an impulse animal has a line about it here. The
    // gate reads the steering model, so it reaches every impulse locomotion and nothing else.
