...
14. **The carrier, 2026-09-19.** Every world with a surface carries one source, and the instrument of the probe reads a bearing to it. The reading has an error and no distance. The globe keeps a wedge per landing, and the source stands where two wedges cross. A landing on the cell of the source shows the thing itself: the wreck of an older survey probe, with the log of its crew, which the lore engine writes from the facts of the world. The search takes three phases: hear, cross, and home. See `docs/issues/34-the-carrier.md` and `docs/source.md`.
...
**The card sees the plant from the height of the eye.** Added the same day. A card held one picture of the side, and it turned about the y axis only. From above that is the wrong picture: a tiered tower read as a column of loose bars with sky between them, and a crown read as a thin band. A first fix held four pictures, from the side and from 30, 60, and 90 degrees, and turned the card to face the eye. That fixed the shape and broke the motion. A card that turns to face the eye shows the same side of the plant from every bearing, and the bearing of a plant under the camera sweeps through half a turn as the camera passes over it. A reader who looked straight down and flew sideways saw every crown under the camera spin in place.

**The card is an impostor.** The bake draws each kind from 25 directions over the upper half of the sphere. The hemi-octahedral map lays that half on a square: the edge of the square is the horizon and its middle is straight down, so a grid of 5 by 5 holds 16 bearings on the horizon, 8 at about 50 degrees, and one straight down. Each view frames the box of the plant as that view sees it, with its own right and up, and the shader builds the same frame for the same direction. The card is a rectangle about the middle of the plant that faces the eye, as wide and as tall as the box seen from the eye. The shader finds the three views nearest the direction of the eye, in the frame of the plant, and weighs them by the place of that direction in their triangle on the grid. For each view it carries the point of the card along the eye onto the picture plane of that view, so the three pictures line up on the plant. The card takes the spin of its plant from the instance matrix, so it shows the side of the plant its mesh shows, and a plant under a passing camera holds still.

**The card is lit when it draws.** The four pictures carried the light of the bake, with the sun behind the eye, and the card dimmed by the angle between the eye and the sun. From above that rule split the view: the plants on one side of the camera went dark and the plants on the other side went bright. The bake now draws the colour of the plant and the direction of its surface, in two textures, and no light. The card is a Lambert material, and the shader hands it that direction, turned by the spin of the plant, in place of the normal of the flat rectangle. The sun and the sky of the ground then light the card as they light the mesh, from every side and at every hour. The empty texels of each normal tile face the eye of their view, and the normals take the nearest texel, so the edge of a plant takes no dark rim.

**What it costs.** A tile takes `frame` times the pixels of the style, up to 112 on the high tier and 64 on the low tier, where `frame` is the longest side of any frame against the height of the plant: 1.15 to 1.61 on `Quasar-579`. The textures take 6 bytes a pixel, so the cards of that patch take 14.3 MB on the high tier, and 4.7 MB on the low tier on `Aurora`. The bake of 50 small draws per kind brings the build of the flora of `Quasar-579` to 84 ms, inside the dive. Three loads of each build, in turn, put the graphics card within about 1 ms of the build with four pictures at every camera, up and down, which is the spread of the runs. The paths above still score 0 at the threshold of 40, and a flight at 250 m that looks straight down scores 0.26 at the threshold of 10.

What stays different: the mesh leans and widens and the card does not, a card casts no shadow, and up close the blend of two views about 45 degrees apart shows a trace of both.
...
Every wedge that covers a fragment adds to a count `n`, and the wash is `1 - pow(0.94, n * n)` of
...
more wedges cover stands out as the answer. One wedge reads 0.06, two read 0.22, three read 0.43,
and four read 0.63. The first build washed `1 - pow(0.92, n)`, where one wedge read 0.08 and
...
A line 0.45 degrees wide on each edge carries the shape of one wedge: the eye finds the cross as the
region the lines close. The line of the newest wedge draws at 0.85 of the accent, and each older one
...
The accent here is the colour of the carrier, and not the accent of the fauna palette.
`pickCarrierColour()` reads the colours of the terrain and of the sea, and it takes the one of ten
saturated candidates that stands farthest from the nearest tenth of the surface. It draws no random
number, so a world keeps its colour. Three candidates are dark, because a bright wash on an ice
sheet shows nothing.
...
**The disc of 14 units, `SOURCE_DISC` in `generate.js`.** The worker flattens a disc of radius 14
