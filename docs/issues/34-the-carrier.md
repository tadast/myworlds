...
- **The box changed hand on 2026-09-19.** The ground box was the mirror of its cell, with z along the v axis of the cell. It now runs z against v; see "The box is right-handed" in `docs/probe.md`. The build of this issue was written against the old box. The merge of the fix corrected `boxPoint()` and `carrierBox()` in `site.js` to take z against v, and it turned the mirror note of `tools/carrier-check.mjs` into check 5, which takes the box map from `worker.js` itself. The wreck needs no change: the worker places it in units of the box.
...
- **The wedges are painted on the terrain, and they are no longer geometry.** The shell of 1.07 failed the wedges too, for the same reason it failed the ring. From the aim camera at 1.11 a wedge stood as a sheet over the ground: the cross of two sheets held a large parallax against the relief, and the reader could not say which cell lay under the cross. `patchCarrierMaterial()` in `carrier-globe.js` now adds the wedges to the terrain material and to the ocean material of the world, and each fragment tests its own direction in the local frame of the planet against the fixes in the uniforms. A wedge therefore lies on the ground it marks at every camera, it turns with the world for free, and it costs no geometry. Every fix still keeps its dot, and the dot drapes on the ground with the ring, at 0.35 of a cell.
- **The search keeps four fixes, and a wedge is narrow.** The first build held `MAX_WEDGES = 8` at an error of 3 to 25 degrees. A reader landed eight times and the globe then stood in eight wide wedges of the accent, over most of a hemisphere, with lines everywhere. The reader could read no cross out of it. Four things changed together on 2026-09-19.
  - **Four fixes.** `MAX_FIXES` in `carrier-store.js` and `MAX_WEDGES` in `carrier-globe.js` are both 4, so the store and the shader hold one set and a fifth landing drops the oldest fix. Four slots cost 44 floats. Four fixes give a cross and one check of that cross.
  - **The error is 2 to 10 degrees.** `CARRIER_ERR = [2, 10]` in `site.js`. With 3 to 25 two far wedges crossed over a quarter of a hemisphere. With 2 to 10 two far fixes cross over a region about 15 cells wide, a third fix from near that region closes it to a few cells, and the range of decision 7 does the rest. That is the three to five landings the plan asks for.
  - **A stored fix takes its numbers again on load.** `carrierAt()` is a pure function of the seed and the cell, so `buildWorld()` in `app.js` passes every loaded fix through it before `makeCarrierGroup()` draws the wedge. The store keeps `{ lat, lon, brg, err }` as it is and the key does not move. A reader who holds the old wide fixes sees them narrow with no clear, and a later tune of `CARRIER_ERR` needs no migration.
  - **The wash shows the overlap.** The wash of `n` wedges over one fragment is `1 - pow(0.94, n * n)` of the way to the accent, and it grows faster than the count: one wedge 0.06, two 0.22, three 0.43, four 0.63. So one wedge alone is almost only its two lines, and the ground two or more wedges cover stands out. The line of an older wedge also draws weaker, at 1, 0.75, 0.55, and 0.4 of `WEDGE_EDGE`, so the eye starts at the last landing. The app still draws no cross and computes none: it only adds the wedges the way ink adds.
...
- **The colour of the carrier stands out against the surface.** The wedges, the dots, and the lamp
  of the mini wreck took the accent of the fauna palette, which a world can also hold in its
  terrain. `pickCarrierColour()` in `carrier-globe.js` now reads the colours of the terrain and of
  the sea. It takes the one of ten saturated candidates that stands farthest from the nearest tenth
  of the surface, and it draws no random number. Three candidates are dark, for an ice sheet. The
  line of a wedge is now 0.45 degrees wide at 0.85 of the colour, and `WEDGE_STEP` is 0.06.

### The checks that stay open

Three acceptance criteria need a person at the machine. No tool can close them.

- **The tune of the error, risk 3.** 3 to 25 degrees was the first value, and it read as a wash over most of a hemisphere. 2 to 10 degrees is the second value. Walk five worlds by hand and count the landings: the median search must take three to five. Change `CARRIER_ERR` in `site.js` if it does not. A change needs no migration of the store, because `app.js` computes the numbers of a stored fix again on every load.
