...
Ten places where the build went another way than the plan. `docs/probe.md` decision 14 holds the
...
- **The search keeps four fixes, and a wedge is narrow.** The first build held `MAX_WEDGES = 8` at an error of 3 to 25 degrees. A reader landed eight times and the globe then stood in eight wide wedges of the accent, over most of a hemisphere, with lines everywhere. The reader could read no cross out of it. Four things changed together on 2026-09-19.
  ...
  - **The wash shows the overlap.** The wash of `n` wedges over one fragment is `1 - pow(0.96, n * n)` of the way to the accent, and it grows faster than the count: one wedge 0.04, two 0.15, three 0.31, four 0.48. So one wedge alone is almost only its two lines, and the ground two or more wedges cover stands out. The line of an older wedge also draws weaker, at 1, 0.75, 0.55, and 0.4 of `WEDGE_EDGE`, so the eye starts at the last landing. The app still draws no cross and computes none: it only adds the wedges the way ink adds.
