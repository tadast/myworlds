...
- **The wash shows the overlap.** Every wedge that covers a fragment adds to a count `n`, and the wash is `1 - pow(0.96, n * n)` of the way from the lit colour to the accent, plus 0.06 of the accent as an emissive share, so a wedge on the night side still reads. The square of the count makes the wash grow faster than the count: one wedge reads 0.04, two read 0.15, three read 0.31, and four read 0.48. So one wedge alone is almost only its two lines, and the ground two or more wedges cover stands out. `wedgeWash(n)` in `carrier-globe.js` is the same formula in JS, and the check tests it.
- **An older line is dimmer.** A line 0.3 degrees wide on each edge carries the shape of one wedge, at `WEDGE_AGE[k]` of 0.5 of the accent: 1, 0.75, 0.55, and 0.4 from the newest wedge to the oldest. The eye then starts at the last landing and works back. The wash does not age. A wedge that fades in counts as the newest one.
...
- `tools/carrier-fix-check.mjs` holds the store, the wedge, the cap, the age, the wash, the fade, and the drape. It builds every test direction with its own copy of the east of the globe and reads it back through `bearingTo()` of `site.js`, so a mirrored east in either file fails the run.
