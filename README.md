...
- `ground.js` holds the ground the probe lands on. The globe is a miniature at 1 unit for the planet radius. The ground is true scale: 1 unit is 1 metre. It keeps a second scene, a second camera, and second controls, and it draws with the same renderer. A sky dome of 5,000 m takes the atmosphere colour, and fog in the same colour runs from 450 m to 750 m at the ground, so the horizon fades and the patch shows no hard edge. The fog opens with the height of the camera, out to 1,260 m and 2,100 m at the ceiling, so the reader looks down on the whole patch from 1,200 m and the rim still fades away before its edge. Later issues put the sea and the fauna in it.
...
- The plants draw at two levels of detail. A plant within 150 m of the camera is the full mesh. A plant beyond it is a card: one vertical quad that carries a picture of the same mesh, baked once per kind at patch load into a target of 64 by 64 pixels with the colours and the sun of the site. The card turns toward the camera around the y axis in the vertex shader, and it dims by the angle between the eye and the sun, so a card and the mesh beside it hold one brightness. The card takes an alpha test and no blending, so it writes depth and sorts with the terrain. Every frame one walk reads the distance of every plant and writes it to the near mesh or to the card mesh, with a band of 10% around the 150 m so a plant at the boundary cannot flicker. The matrices are built once at load and the walk only copies them, so it costs about 0.4 ms for 20,000 plants. A forest site at 30 m over the ground draws in about 4.4 ms of card time, terrain included.
