...
- `ground.js` holds the ground the probe lands on. The globe is a miniature at 1 unit for the planet radius. The ground is true scale: 1 unit is 1 metre. It keeps a second scene, a second camera, and second controls, and it draws with the same renderer. A sky dome of 5,000 m takes the atmosphere colour, and fog in the same colour runs from 450 m to 750 m at the ground, so the horizon fades and the patch shows no hard edge. The fog opens with the height of the camera, out to 1,260 m and 2,100 m at the ceiling, so the reader looks down on the whole patch from 1,200 m and the rim still fades away before its edge. Later issues put the sea, flora, and fauna in it.
