...
const DRAG = 6;              // pixels: a pointer that moves further than this is a drag, not a tap
...
export class GroundFauna {
  ...
  // ---------------------------------------------------------------- the inspector click
  // Issue 06 owns the ground camera and the ground click. Until the two meet, this file wires its
  // own tap, so a reader on the ground can open a card.
  _bind() {
    if (!this.canvas || !this.onInspect) return;
    this._onDown = (e) => { this._down = [e.clientX, e.clientY]; };
    this._onUp = (e) => {
      const d = this._down; this._down = null;
      if (!d || Math.hypot(e.clientX - d[0], e.clientY - d[1]) > DRAG) return;
      const kind = this.pickAt(e.clientX, e.clientY, e.pointerType === 'touch' ? PICK_TOL * 1.5 : PICK_TOL);
      if (kind !== null) this.onInspect(kind);
    };
  ...
  pickAt(px, py, tolerance = PICK_TOL) {
    ...
    for (const m of this.members) {
      ...
      if (d < bestD) { bestD = d; best = m.inst.userData.kind; }
    }
    return best;
  }
