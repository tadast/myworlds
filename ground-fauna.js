...
export class GroundFauna {
  ...
  constructor({ result, world, tier, heightAt, camera, canvas, lod, sunDir, night }) {
    ...
    // Every hook takes a plain point of the patch, in metres. A mover holds its place as an offset
    // from its home, and the home of a group is its anchor, so the mover carries that anchor as
    // st.ox and st.oz and adds it back before it asks. Without it an impulse animal would read the
